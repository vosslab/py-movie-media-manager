...
def test_transport_fetch_html_timeout():
	"""fetch_html raises ConnectionError on timeout."""
	import moviemanager.ui.imdb_browser_transport
	fake = FakeTransport()
	# bind the real fetch_html method to our fake transport
	bound_fetch = (
		moviemanager.ui.imdb_browser_transport
		.ImdbBrowserTransport.fetch_html.__get__(fake)
	)
	with pytest.raises(ConnectionError, match="timed out"):
		bound_fetch("https://www.imdb.com/title/tt0109445/", timeout_sec=0)


#============================================
def test_transport_fetch_html_load_failure():
	"""fetch_html raises ConnectionError when page load fails."""
	import moviemanager.ui.imdb_browser_transport
	fake = FakeTransport()
	...
	with pytest.raises(ConnectionError, match="load failed"):
		bound_fetch("https://www.imdb.com/title/tt0109445/", timeout_sec=5)
...
def test_transport_on_html_received_sets_event():
	...
	bound_handler = (
		moviemanager.ui.imdb_browser_transport
		.ImdbBrowserTransport._on_html_received.__get__(fake)
	)
...
def test_transport_on_load_finished_failure():
	...
	bound_handler = (
		moviemanager.ui.imdb_browser_transport
		.ImdbBrowserTransport._on_load_finished.__get__(fake)
	)
	bound_handler(False)
	assert fake._event.is_set()
